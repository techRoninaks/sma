<?php
    ...
    $productQuery = "SELECT p.category_id,p.active_status, c.parentid, p.prodid, p.created_date, p.sold_count, p.avg_prcessing_time, p.avg_confrmn_time,p.shop_id,sl.seller_name, sh.shopname, p.name, p.short_desc, p.base_price, (SELECT o.percentage from $DB_SMA_PR.offer o WHERE o.id = p.offer_id) as percentage, p.active_status, p.has_rfq, p.rating, p.has_order_confmn, p.has_instant_buy
    FROM $DB_SMA_PR.product p, $DB_SMA_PR.category c, $DB_SMA_USR.seller sl, $DB_SMA_USR.shop_details sh, $DB_SMA_USR.shipping_location_shop sls, $DB_SMA_PR.prod_shipping_price psp
