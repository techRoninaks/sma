<?php
    ...
    if($faqNum==1){
        ...
        $sql_query1="SELECT f.text,f.type_id from faq f join faq q on abs(f.type_id)=q.type_id WHERE f.prodid=$prodId AND f.text LIKE '%$filter%' ";
        ...
        while($dataSz!=0){
            ...
            $sql_query = "SELECT f.text,f.type_id from faq f join faq q on abs(f.type_id)=q.type_id WHERE abs(f.type_id) =$typeId AND abs(q.type_id)=$typeId ";
