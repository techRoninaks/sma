<?php
    ...
    $sql_query7 = "SELECT count(*) as Ordercount FROM `purchase_order` where customerid = $userId && order_status = 'pending payment' ";
    $result7 = mysqli_query($con2, $sql_query7);
    $row7 = mysqli_fetch_array($result7);
    $orderCount=$row7["Ordercount"];
    
    $sql="SELECT `orderid` FROM `purchase_order` WHERE customerid = $userId && order_status = 'pending payment' ORDER BY `orderid` ASC ";
    ...
    while($j < $orderCount){
        ...
        $sql_query1="SELECT `created_date`,`customerid`,`order_status`,`delivery_date` FROM `purchase_order` WHERE orderid = $orderId && order_status = 'pending payment'";
