<?php
    ...
    $sql_query = "select * from trending_products limit $tagNum offset $offset";
    $result = mysqli_query($con2,$sql_query);
    $data = array();
    $count = 0;

    while($row = mysqli_fetch_array($result)){
        $data[$count++] = array("shopId"=>$row["shop_id"],"sellerName"=>$row["seller_name"],
        "prodId"=>$row["prodid"],"prodName"=>$row["name"],
        "desc"=>$row["short_desc"],"basePrice"=>$row["base_price"],
